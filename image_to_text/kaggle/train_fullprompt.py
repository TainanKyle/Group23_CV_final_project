...
image_dir = "/home/gpl_homee/indoor_scene/SceneTex/image_to_text/kaggle/interior/img"
text_dir = "/home/gpl_homee/indoor_scene/SceneTex/image_to_text/kaggle/interior/style"
test_image_dir = "/home/gpl_homee/indoor_scene/SceneTex/image_to_text/lsun/test/img"
test_text_dir = "/home/gpl_homee/indoor_scene/SceneTex/image_to_text/lsun/test/style"
checkpoint_path = "checkpoint_lowest_loss.pth"
...
class ImageTextDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        text = "a " + text + " style room, " + a_prompt
        # return image, text
...
model = TwoLayerMLP(input_dim=768*2).to(device)
...
if is_Train:
    for epoch in range(epochs):
        ...
        # concatenate image embeddings with a_prompt embeddings
        with torch.no_grad():
            text_inputs = clip_processor(text=[a_prompt], return_tensors="pt", padding=True, truncation=True).to(device)
            a_prompt_embedding = clip_model.get_text_features(**text_inputs).to(device)
        ...
        for image_paths, texts in tqdm(dataloader, desc=f"Epoch {epoch+1}/{epochs}"):
            ...
            with torch.no_grad():
                ...
                a_prompt_embeddings = a_prompt_embedding.repeat(image_embeddings.shape[0], 1)
                image_embeddings = torch.cat([image_embeddings, a_prompt_embeddings], dim=-1)
        ...
        if epoch == 0:
            ...
            if running_loss < lowest_loss:
                lowest_loss = running_loss
                torch.save(model.state_dict(), "checkpoint_lowest_loss.pth")

    # save the model
    torch.save(model.state_dict(), f"checkpoint_epoch_{epochs}.pth")
...
if is_Test:
    ...
    with torch.no_grad():
        ...
        text_inputs = clip_processor(text=[a_prompt], return_tensors="pt", padding=True, truncation=True).to(device)
        a_prompt_embeddings = clip_model.get_text_features(**text_inputs).to(device)
